...
function fillHairMenu() {
  hairItem = new UIMenuListItem(
    'Hair',
    "Your character's hair.",
    new ItemsCollection(Data.hairList[currentGender].map((h) => h.Name))
  );
  ...
  hairHighlightItem = new UIMenuListItem(
    'Hair Highlight Color',
    "Your character's hair highlight color.",
    new ItemsCollection(hairColors)
  );
  creatorHairMenu.AddItem(hairHighlightItem);

  eyebrowColorItem = new UIMenuListItem(
    'Eyebrow Color',
    "Your character's eyebrow color.",
    new ItemsCollection(hairColors)
  );
  creatorHairMenu.AddItem(eyebrowColorItem);

  beardColorItem = new UIMenuListItem(
    'Facial Hair Color',
    "Your character's facial hair color.",
    new ItemsCollection(hairColors)
  );
  ...
  lipstickColorItem = new UIMenuListItem(
    'Lipstick Color',
    "Your character's lipstick color.",
    new ItemsCollection(lipstickColors)
  );
  creatorHairMenu.AddItem(lipstickColorItem);

  chestHairColorItem = new UIMenuListItem(
    'Chest Hair Color',
    "Your character's chest hair color.",
    new ItemsCollection(hairColors)
  );
...
let genderItem = new UIMenuListItem(
  'Gender',
  '~r~Changing this will reset your customization.',
  new ItemsCollection([ 'Male', 'Female' ])
);
  ...
  switch (index) {
    ...
    case 6:
      ...
      mp.events.callRemote(
        ...
        JSON.stringify(appearanceData),
        JSON.stringify(hairAndColors)
      );
      break;
...
let similarityItem = new UIMenuListItem(
  'Resemblance',
  'Similarity to parents.\n(lower = feminine, higher = masculine)',
  new ItemsCollection(similarities)
);
let skinSimilarityItem = new UIMenuListItem(
  'Skin Tone',
  "Skin color similarity to parents.\n(lower = mother's, higher = father's)",
  new ItemsCollection(similarities)
);
...
for (let i = 0; i < Data.appearanceNames.length; i++) {
  ...
  let tempAppearanceOpacityItem = new UIMenuListItem(
    Data.appearanceNames[i] + ' Opacity',
    '',
    new ItemsCollection(opacities)
  );
...
creatorAppearanceMenu.ListChange.on((item, listIndex) => {
  let idx =
    creatorAppearanceMenu.CurrentSelection % 2 == 0
      ? creatorAppearanceMenu.CurrentSelection / 2
      : Math.floor(creatorAppearanceMenu.CurrentSelection / 2);
